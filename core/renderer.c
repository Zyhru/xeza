...
void renderer_create_cube(renderer_t* r) {
    ...
    unsigned int index_buffer[] = {
      // Front face  (H=0, G=2, F=6, E=7)
      0, 2, 6,
      0, 6, 7,

      // Back face   (D=4, C=1, B=5, A=3)
      4, 1, 5,
      4, 5, 3,

      // Left face   (A=3, D=4, H=0, E=7)
      3, 4, 0,
      3, 0, 7,

      // Right face  (C=1, B=5, F=6, G=2)
      1, 5, 6,
      1, 6, 2,

      // Top face    (D=4, H=0, G=2, C=1)
      4, 0, 2,
      4, 2, 1,

      // Bottom face (A=3, E=7, F=6, B=5)
      3, 7, 6,
      3, 6, 5
    };
    ...
    for(size_t i = 0; i < index_length; i++) {
        printf("Element appended to IB: %d", index_buffer[i]);
        list_append(cube.index_buff, (unsigned int *)&index_buffer[i]);
    }

    // TODO: Think of how to correctly append the index buffer
    //memcpy(cube.index_buff->addr, index_buffer, sizeof(index_buffer));
