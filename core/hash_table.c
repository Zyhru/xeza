...
bool ht_key_eq(vertex_key_t k, vertex_key_t q) {
...
void ht_add(ht_t* table, entry_t* entries, vertex_key_t key, unsigned int value, size_t capacity) {
...
void ht_print_table(ht_t* table) {
    ...
    for(int i = 0; i < table->capacity; ++i) { 
        if(table->entries[i].occupied) {
            vertex_key_t key = table->entries[i].key;
